...
class TreatmentPlanWidget extends StatefulWidget {
  ...
  @override
  State<TreatmentPlanWidget> createState() => _TreatmentPlanWidgetState();
}

class _TreatmentPlanWidgetState extends State<TreatmentPlanWidget> {
  Data data = Data();
  List<Items> items = [];
  ...
  void updatedItems(index) { 
    if(items[index].cost == null || items[index].cost == '') items[index].cost = '0.0'; 
    ...
    items[index].total = (double.parse(items[index].cost.toString()) * double.parse(items[index].unit.toString())).toString(); 
    items[index].discountAmount = ((items[index].discountValue ?? 0) * double.parse(items[index].cost.toString()) / 100).toString();
    items[index].taxAmount = ((items[index].taxValue ?? 0) * double.parse(items[index].cost.toString()) / 100).toString(); 
    updated();  
  }
                          ...
                          XButton(
                            ...
                            onPressed: () async {
                              ...
                              final response = await context.read<MainCubit>().postRes(
                                'sales/customer/${widget.customerId}/treatment-plans/create',
                                model.toJson(),
                                context
                              );
