...
class HrCubit extends Cubit<HrState> {
  ...
  Future<CustomersModel> getCustomers({limit = 10, next = 0, query}) async {
    try {
      loading = true;
      emit(const LoadingHrState()); 
      final res = await _repository.get(limit, next, 'sales/customers', query: query);
      loading = false; 
      emit(const LoadedHrState());
  ...
  Future<VendorModel> getVendors({limit = 10, next = 0, query}) async {
    try {
      loading = true;
      emit(const LoadingHrState()); 
      final res = await _repository.get(limit, next, 'purchase/vendors', query: query);
      loading = false; 
      emit(const LoadedHrState());
  ...
  Future<ResponseModel> createDepartment(Map<String, dynamic> map,{id}) async {
   ...
   Future<ResponseModel> createVendor(Map<String, dynamic> data) async {
    try {
      emit(const LoadingHrState());
      log(data.toString());  
      var res = await _repository.post(data, 'purchase/update-or-create-vendor', multipart: true);
    ...
    try {
      emit(const LoadingHrState());
      var res = await _repository.post(data, 'purchase/create-or-update-purchase-order', multipart: true);
   ...
   Future<ResponseModel> creatExpense(Map<String, dynamic> data) async {
    try {
      emit(const LoadingHrState());
      log(data.toString());  
      var res = await _repository.post(data, 'purchase/update-or-create-expense', multipart: true);
 ...
 Future<ExtraModel> getExtra() async {
  ...
  Future<RolesModel> getRoles({limit = 10, next = 0, search = ''}) async {
   ...
   Future<ResponseModel> createEmployeeType(Map<String, dynamic> map,{id}) async {
    try {
      emit(const LoadingHrState());
